...
if(isset($_POST['saveupdates']))
{
  ...
  $sql4="update tbladmin set FirstName=:firstname2,LastName=:lastname2,MobileNumber=:mobilenumber2,MyAddress=:address2 where ID=:aid";
  $query4 = $dbh->prepare($sql4);
  $query4->bindParam(':firstname2',$fName2,PDO::PARAM_STR);
  $query4->bindParam(':lastname2',$lName2,PDO::PARAM_STR);
  $query4->bindParam(':address2',$address2,PDO::PARAM_STR);
    ...
    if($query->rowCount() > 0)
    {
      ...
      { 
        ...
        <div class="form-group">
          ...
          <input type="text" name="address2" class="form-control" id="address2" value="<?php  echo $row->MyAddress;?>"  required>
        </div>
        <?php $cnt=$cnt+1;
      }
    } ?>
